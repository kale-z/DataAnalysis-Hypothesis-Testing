...
"""since the first Hypothesis is (Time Spent VS GPA),
We take the last 2 columns, which we need, and get rid of the other columns"""

# Slicing the Data
Data_First = []
for row in Data:
    Data_First.append([row[2], row[3]])
...
R = ((len(Data_First[1:]) * XY) - (X * Y)) / (sqrt(((len(Data_First[1:]) * Xsquared) - (X ** 2)) * (len(Data_First[1:]) * Ysquared) - (Y ** 2)))
...
print("H0: There is NO Significance Correlation between Spent Time on social media and students's GPA [H0: P = 0]")
print("H1: There is a Significance Correlation between Spent Time on social media and students's GPA [H0: P ≠ 0]\n")
...
# Part [ 2 ] - Regression:
# Correlation Coefficient proof, Scatter Plot
colors = np.random.rand(len(Data_First[1:]))
area = np.pi * (15 * np.random.rand(len(Data_First[1:])))*2
plt.figure(2)
plt.scatter([row[0] for row in Data_First[1:]], [row[1] for row in Data_First[1:]], s=area,c=colors, alpha=0.5)
x=[pair[0] for pair in Data_First[1:]]
y=[pair[1] for pair in Data_First[1:]]
m, b = np.polyfit(x,y, 1)
Xnew = []
for i in range(len(x)):
    Xnew.append((x[i] * m)+b)
plt.plot(x, Xnew,'c-')
plt.title("Data Distribution for [Time Spent vs Student's GPA]")
plt.xlabel("Time Spent on Social Media")
plt.ylabel("GPA of students")
plt.show()

# Computing the Residuals
inter, slope = thinkstats2.LeastSquares([element[0] for element in Data_First[1:]],[element[1] for element in Data_First[1:]])
Residuals = thinkstats2.Residuals([element[0] for element in Data_First[1:]],[element[1] for element in Data_First[1:]],inter,slope)
